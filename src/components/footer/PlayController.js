...
function PlayStatusCom(props) {
  ...
  return <IconFont type={type} style={{ color: '#fff', fontSize: '24px', cursor: 'pointer' }} onClick={() => props.onClick()} className="webkit-no-drag" />
}
...
function SetVolumeCom(props) {
  ...
  const menu = (
    <div style={style}>
      <Slider vertical max={10} min={0} step={1} defaultValue={props.defaultValue * 10} value={currentVolume} onChange={(value) => setVolume(value)} />
    </div>
  )
  ...
  return (
    <Dropdown overlay={menu} trigger={['hover']} placement="topCenter">
      <Space className="webkit-no-drag">
        <IconFont style={{ fontSize: '16px' }} onClick={() => setVolume(0)} type={currentVolume === 0 ? 'icon-mute' : 'icon-yinliang'} className="webkit-no-drag" />
      </Space>
    </Dropdown>
  )
}
